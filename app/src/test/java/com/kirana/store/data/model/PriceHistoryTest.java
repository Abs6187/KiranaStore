...
public class PriceHistoryTest {
    ...
    public void paramConstructor_assignsAllFields() {
        PriceHistory h = new PriceHistory(7, 220.0, 200.0, "voice", "Atta 175 rupees kar do");

        assertEquals(7, h.productId);
        assertEquals(220.0, h.salesPrice, 0.001);
        assertEquals(200.0, h.purchasePrice, 0.001);
    ...
    public void paramConstructor_acceptsAllThreeSources() {
        ...
        assertNotNull(new PriceHistory(1, 90.0, 80.0, "manual", null));
        assertNotNull(new PriceHistory(1, 92.0, 0, "voice", "dal 92"));
        assertNotNull(new PriceHistory(1, 95.0, 85.0, "ocr_scan", "receipt row"));
    }
}
